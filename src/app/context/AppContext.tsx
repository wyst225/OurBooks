...
export interface RentedBook extends Book {
  rentedUntil: Date;
  progress: number;
}
...
interface AppContextType {
  ...
  cart: CartItem[];
  addToCart: (book: Book, rentalDays: number) => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const checkout = () => {
    const newRentedBooks: RentedBook[] = cart.map((item) => ({
      ...item.book,
  ...
  const replyToMessage = (messageId: string, text: string) => {
    setSupportMessages((prev) =>
      prev.map((msg) =>
        msg.id === messageId
          ? {
